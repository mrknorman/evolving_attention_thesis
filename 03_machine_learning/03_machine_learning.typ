...
```,
caption : [_Python @python ._ Function to initialise TensorFlow @tensorflow tensors to store the artificial neuron's parameters, $accent(theta, arrow)$. In the case of MNIST @mnist digit recognition, ten neurons are being trained, so we have ten bias values, $accent(b, arrow)$, and the input images are of dimension $28 times 28 = 784$. Therefore, our weights matrix, $W$, is shaped `[784, 10]`.]
