...
The elements inside these weights matrices are the only tunable parameters that are learned during the model training process. During model training, the weights will adapt so that they can generate effective query, key, and value vectors that allow for proficient model function. Since this is a neural network and these are learned weights, multiplication by these weights matrices is equivalent to the application of a dense layer with no bias values.
...
where $sigma$ is the Softmax function; see @softmax-sec, and $d_k$ is the number of elements in the key vector, used to scale the value so that differences remain large when $d_k$ is large. This scaling was not a part of the original dot-product attention approach and was added by Vaswani _et al._ @attention_is_all_you_need, it has since become a common feature in attention layers.

#figure(
    image("alignment_function_diagram.png",  width: 100%), 
    caption: [Illustration of the operation of how the alignment function utilizes the query and key vectors to produce alignment scores for each sequence element. In dot-product attention, this is achieved using @alignment-eq. Note that the numbers used here are for illustratory purposes only and not extracted from a real model.]
) <alignment-diagram>
...
Where again, $sigma$ is the Softmax function. Combining @attention-eq-general with @weight-1-eq, @weight-2-eq, and @weight-3-eq gives a mapping between the attention input matrix, $X$, and the attention output matrix, $Y$.
...
It should be noted that in practice, the entire query key and value matrices for each head are calculated at once with the same large weights matrices which produce output matrices that can then be split along the feature vectors and fed into the individual heads for the alignment scores calculation and vector summation. This is done to reduce the number of matrix multiplications required for the layer as a whole.
...
Within transformers and other similar architectures, multi-head attention layers are often paired with a number of complementary layers within a residual block. The input and output matrices of this block usually have identical shapes so that the block can be repeated, $N$ times without having any intermediate reshaping layers. Attention blocks typically feature a number of dense layers with activation functions in order to perform non-linear computation, regularisation methods such as dropout and batch normalisation, and a residual skip connection wherein the block input is added to the block output, in order to reduce the vanishing gradient problem that can occur in very deep networks; see @attention-block-diagram.
...
When expanding our literature search to include models outside of CNNs we encounter a large body of work that has expanded greatly in recent years, even since this work was carried out. The volume presented makes it difficult to perform a complete review as it would be beyond the scope of this document. A review article @gw_machine_learning_review is suggested for a more extensive overview. 

An architecture commonly applied to time series problems is the Recurrent Neural Network. Recurrent neural networks have an internal state determined by previous inferences, and thus, they can retain some information about previous data. In many ways, RNNs were the predecessor to Transformer models, largely because they are able, in some way, to make inferences from global information rather than being limited by the receptive fields of convolutional filters. There have been many studies on the application of RNNs to the transient detection problem. 

One of the most widely applied RNN subtypes is the Long Short Term Memory (LSTM) network, which utilizes network controllable gates, which, based on learned computation, can control what data is preserved between inferences. Though this is not a permanent memory solution, as LSTMs will eventually lose recall ability from temporally distant inferences, it has the potential to store information across many thousands of inferences, hence the name Long Short-Term Memory. Bidirectional LSTMs, which are used almost exclusively throughout the literature, operate on sequences in both directions, allowing the model to see information both after and before the segment currently being analyzed. 

There have been several attempts to apply LSTMs to CBC detection. Schmitt _et al._ @bidirectional_lstm_comp_2 performed a comparison of multiple network architectures including bidirectional LSTMs and found that Temporal Convolutional Networks, which utilize a combination of convolutional and recurrent layers outperformed both the CNN employed by George _et al._ @george_huerta_cnn and traditional matched filtering approaches. Nigam _et al._ @bidirectional_lstm_comp perform a comparison of many different machine learning methods not limited to artificial neural networks or deep learning, but results presented are limited and difficult to compare, they found CNNs and RNNs simililarly effective. Chatterjee _et al._ @bidirectional_lstm present a model consisting of multiple convolutional and recurrent layers. They find good performance and were able to detect all events with a high score. They also present their model as a method for denoising signals for further analysis. They lack a robust discussion of False Alarm Rates. Lin _et al._ @bayesian_lstm present an interesting method that utilizes Bayesian networks along with LSTMs to incorporate uncertainty into the network. This allows the model to present its result along with variable confidence, though it demonstrates comparable predictive power it has a high false alarm rate, and they suggest that a larger training dataset is required to improve results.
...
Utilizing a different RNN architecture, @bidirectional_gru Zhang _et al._ demonstrated a Bidirectional Gated Recurance Unit (GRU) model, in comparison to a CNN and a Fourier Convolutional Neural Network (FCNN). They found the GRU to be the best-performing model and achieved an impressive performance of 89.6% accuracy at a False Alarm Rate (FAR) of $4.88 times 10^(−4)$ on a pool of injections with SNRs between 5 and 20.

RNNs can be computationally expensive to train and run compared to other methods like CNNs, there have been several papers focused on performance improvements @accelerating_rnns @optimising_rnns_2.
...
As a newer architecture, and one that is slightly less obvious to apply to time series data, since it is primarily used in NLP, there has been less attention on the application of transformers and attention-based models to transient detection problems. @coherent_snr_attention uses attention layers in a coherence based detection model. 


 However, there has not been a significant investigation into the use of transformers for such a purpose, with only this paper by Zhao et al known at this time, which focuses on the problem of space-based detection. https://arxiv.org/pdf/2207.07414.pdf
 
== Skywarp Method <skywarp-method>

=== Skywarp <skywarp-model>

We have investigated a number of attention-based models and compared their performance to convolutional models as well as other traditional analysis techniques. We have investigated a fully attention-based model, as well as a combined convolutional-attention model.

For the pure CNN model, we adapted the model from George _et al._ @george_huerta_cnn. Architecture is as described in figure \ref{fig:cnn}.
