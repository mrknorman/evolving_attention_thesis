...
Convolutional Neural Networks (CNNs), though effective, have not been the stars of the machine-learning world for a few years now. Just as AlexNet @image_classification paved the way for the era of CNNs, and the surge to prominence of artificial neural networks as problem-solving solutions, a similar step change occurred when the confluence of several different technologies which led to the development of Large Language Models (LLMs), most notably of which in recent years has been the Generative Pretrained Transformer (GPT) of ChatGPT fame, developed by OpenAI, though it should be noted that this was far from the first LLM.  Although CNN was extremely successful at solving previously intractable problems in the image and audio domain, there were still many challenges remaining within Natural Language Processing (NLP), the area of study relating to data analysis of the text domain. The text domain differs from that of time series, in that rather than vectors of continuous numerical values, it consists of sequences of discrete tokens, whether you divide it all the way into individual characters or constrain the tokens further to unique individual words. There had been some work to attempt these problems with CNNs and Recurrent Neural Networks, but it was only through the application of multiple different insights including Word2Vec, attention mechanisms, and skip connections, that the start of the avalanche of progress we have since seen in this area, began. The groundbreaking paper by Google, Attention is All You Need by Vaswani _et al._, @attention_is_all_you_need first introduced the transformer architecture to the world.

Following the success of the transformers in NLP, there has been much research into the application of attention-based models to other domains, including image and audio processing. Notably, AlphaFold, a project by Google Deepmind, successfully solved the protein folding problem. Transformers have proven effective, scalable solutions due to their highly parallelizable nature, and new attention-based architectures such as generative diffusion models have seen great success in image-to-text generation problems, as is seen in products like Stable Diffusion, Midjourney, and Dall-E. Gravitational-wave astronomy has also seen the application of a large number of machine-learning approaches; however, there exists a certain time lag between the advent of new techniques and their application to gravitational wave datasets. Since this research was originally carried out, there gap has narrowed, but nonetheless, it remains a relatively unexplored area that may prove fruitful in the future. In this section, we introduce Skywarp, an attention-based model for the detection of gravitational waves produced by Compact Binary Coalescences (CBCs).

The distinguishing feature of the transformer architecture is the use of attention. Attention is a technique that can determine the information value of data elements in a series, crucially including the relative information derived contextually from the value and position of all other data elements within the series. As opposed to convolutional layers, which learn feature-extracting filters that are convolved with the input data, attention heads learn a weighting for each data element, identifying the information interactions between elements. One key advantage of this method is that attention is computed globally, whereas convolutional layers only use a local context dictated by their receptive field. This makes attention an ideal candidate for analyzing time-series data, wherein important contextual information can be located at a significant temporal distance from any given datum. Recurrent neural networks share many of these properties; however, they make use of an internal state, which means they are much harder to parallelize and therefore scale, and their knowledge of data outside of their receptive field is uni-directional, as opposed to transformers, which are bi-directional. In addition, self-attention layers are more general than convolutional layers, as it is proven that a self-attention layer can generalize any convolution.

This section is organized into the following structure. First, we will give a brief technical overview of the concepts and structure of the transformer architecture in @attention-method, including a discussion of tokenization, embedding, attention, and how these elements are assembled into transformers proper. Then we will review the small amount of relevant literature that has not already been discussed in @skywarp_review. Next will provide details on the models and training procedures we used to train Skywarp in @skywarp-method. We will show the validation results from our trained models in @skywarp-results, before finally in @skywarp-discussion, we discuss the importance of these results and investigate how the method employed by the transformer models differs from that used by CNNs by comparing the attention and convolution maps.
