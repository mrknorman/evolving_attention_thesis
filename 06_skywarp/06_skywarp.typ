...
The models we describe in this section are novel in that they utilize attention mechanisms @attention_1 @attention_2, a type of differentiable memory in which a global context vector is learned over an input sequence, $X = [accent(x, arrow)_1 ... accent(x, arrow)_i ... accent(x, arrow)_n]$. The attention mechanism aims to embed global context locally; in order to do this, a comparison must be made between each element of the sequence and (in the case of self-attention) each other element of the same sequence. It is trivial to see that not every element in every sequence will be equally contextual to each other and that the contextual dependence will depend on the information being extracted. In this way, one learns intra-sequence relations; long-term dependencies are captured because the entire input sequence is used to compute a single element of the output sequence. 

The question becomes, how can we calculate the attention score? We can use an analogous problem to demonstrate the principle. In search and retrieval tasks, such as a search engine query, the user, in this case, a human, must generate a *query* phrase that can be used to find relevant information. This query phrase will not contain the entire information content of whatever document we are attempting to discover, if it did then we would not need to perform the search. Instead, it is generated using words and phrases that are associated with the information we are searching for. The search engine might then perform further processing, taking this human-generated query and optimizing it further for better results. The search engine then has the unenviable task of searching through its entire library to find documents that might have information relevant to the query. The first instinct might be to look through every document and check to see if there are words and phrases in that document that match the content of the query. Immediately, we can tell that this will quickly become infeasible if the library is large, and/or contains large documents --- the process of searching would very quickly become very expensive. Instead, the search engine could have preprocessed these files, and in a similar manner to how the query was generated, it could pick out the key information content of each document in a distilled that contains the information that it is most likely that a userhas entered as their query. It generates a *key* or keys for that document, which can be checked against queries much more efficiently than searching the entire content. Finally, the *value* of the information that the end user extracts from whatever document is returned, will not necessarily equate to the entire information content of the document. Depending on what information the user was originally searching for, and hence what query they entered into the search bar, they might only read a particular chapter of a book, or, even more specifically than that, they might only retain certain parts of information from that chapter that are relevant to their needs. During a search session, a user might enter a single query which matches well with multiple keys that return multiple documents in which the user reads and sumarises parts of the information in each document to gain new knowledge on whatever the original subject of their query was.

This analogy introduces the three key information concepts of the query, key, and value. We can use these concepts to build a deep learning layer that can, for every element of our input sequence search through each element in the sequence and extract relevant information than can then be embed into that element, in a similar manner to how we can embedd the elements position using positional encoding. In attention layers, query, $accent(q, arrow)_i$, key, $accent(k, arrow)_i$, and value $accent(v, arrow)_i$ vectors are generated for  each sequence element $accent(x, arrow)_i$, forming three matrices for the sequence as a whole: $Q$, $K$, and $V$. We create these matricies by applying three projection matricies to the input matrix, X, the query, $W_q$, key, $W_k$, and value, $W_v$, matries. $Q$, $K$, and $V$ are generated with

$ Q = W_q dot X, $
$ K = W_k dot X, $
$ V = W_v dot X. $

The elements inside these weights matricies are the only tunable parameters that are learned duing the model training process. During model training, the weights will adapt so that they can generate effective queries, keys, and values for the attention layer to function effectively. The nature of attention layers makes it much more difficult to draw attention layers usin artificial neuron connection diagrams as we have previously with perceptrons and CNNs since the information flow is more complex. However we can still attempt to visulise the interaction the interaction between the various vectors as interacting function; see @weights_matricies.

#figure(
    image("weights_matricies.png",  width: 100%), 
    caption: [Generation of query, key, and value vectors for each element in the input sequence of length, $N$. Before attention scores are calculated, each input vector, $accent(x, arrow)_i$ is in turn dotted with the learned query, $W_q$, key, $W_k$, and value, $W_v$, weights projection matricies to produce a query, $accent(q, arrow)_i$, key, $accent(k, arrow)_i$, and value $accent(v, arrow)_i$ vector for the input element, $accent(x, arrow)_i$. This operation is equivilent to the multiplication of the projection matricies and the input matrix, X, to produce the query, $Q$, key $K$, and value $V$ matricies. The key takeaway is that the only tunable parameters are contained int he weights matricies, which act as projection functions to convert the input vector into functional vectors.]
) <weights_matricies>

#figure(
    image("q_k_v.png",  width: 60%), 
    caption: [Example of query, key, and value vectors generated for the sentence "The quick brown fox jumped over the lazy dog.", after tokenisation and embeding. Each vector in the embedded input sequnece generates its own query, key, and value vector.]
) <qkv_diagram>

The query, key, and value matrices are used to calculate the attention scores for each element; see @qkv_diagram for an illustrative example of the projection matricies applied to the example sentence. The aim of the attnetion method is to collect all relevent information about a given token within that token, extracting the information content from the position and meaning of the surrouding words. Understandably, language does not have words for every possible concept, instead it relies on combinations of words to provide many more concepts than words could alone. For example, language could have developed a single word for "lazy-dog" and "quick-fox"; but you would soon end up with an extrodinarily large vocabulary assuming that new words were invented rather than just cheating and compouding words with a hypen. If we wanted to include much more complex concepts like "quick-brown-fox-that-jumped-over-lazy-dog" and "lazy-dog-that-has-had-quick-brown-fox-jump-over-it", the number of potential concepts becomes huge. Within the vector space however, we are not limited by discretised tokens, and such values can all exist in a highly multi-dimensional space. Attention layers essentially attempt to assemble those complex words.

In order to assemble these new vectors with embedded contextual meaning, ,
