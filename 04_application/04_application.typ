...
To compute the PSD with enough computational speed to perform rapid whitening and SNR, $rho_"opt"$, calculation during model training and inference, an existing Welch method from the SciPy scientific Python library @scipy was adapted, converting its use of the NumPy vectorised CPU library @numpy to the TensorFlow GPU library @tensorflow; this converted code is seen in @psd_calculation.
...
The $h_op("rss")$ is most often used during burst analysis as a measure of the detectability, magnitude, and significance of burst transients. Within CBC detection, $rho_"opt"$ is often preferred. Whilst $h_op("rss")$ is a simple and convenient measure, it ignores noise, so it cannot by itself tell us if a signal is detectable.
...
For the work during this thesis, we have used a TensorFlow @tensorflow implementation for calculating the $rho_"opt"$ of a measure. This implementation is show in @snr_calculation.
...
For the single CBC case, cuPhenom waveforms with masses drawn from uniform distributions between #box("5.0" + h(1.5pt) + $M_dot.circle$) and #box("95.0" + h(1.5pt) + $M_dot.circle$) for the mass of both companions, and between -0.5 and 0.5 for the dimensionless spin component are injected into the noise and scaled with optimal SNR values taken from a uniform distribution of between 8.0 and 15.0 unless explicitly stated.
...
In order to determine the score threshold of a model for a given FAR, we can run that model over a sufficiently large FAR dataset, sort these scores from smallest to highest, and then assign each score an equivalent FAR. For example, if we sorted the scores from lowest to highest, and the first score was above the score threshold, then the FAR would be $1.0 / d_op("example") "Hz"$, where $d_op("example")$ is the length of the input example in our case #box("0.1"+ h(1.5pt) + "s"). If we set the threshold to be smaller than the smallest score this would mean that all the scores are above the threshold, therefore the model would produce a false alarm every time it ran. If the second sorted score was above the threshold but not the first, then all but one of the examples would be a false alarm, therefore the score would be  $(d_op("total") - d_op("example")) / d_op("total") times 1.0 / d_op("example") "Hz"$, $d_op("total")$ is the total duration of examples in the validation set. This gives a general formula for the y-axis.

$ y = (d_op("total") - i d_op("example")) / d_op("total") times 1.0 / d_op("example") "Hz" ,$ <FAR_index_calc>

where i is the x-axis index. We see this plotted in @perceptron_single_far. 

#figure(
  image("perceptron_single_far_curves.png", width: 100%), 
  caption: [Perceptron False Alarm Rate (FAR) curves. This plot was created by running each of our 14 models over a pure noise validation dataset of $1^5$ noise examples. A relatively small number of examples were used due to the inaccuracy of the models during training which suggested that they would not be able to reach low FAR scores and thus would not necessitate a larger validation dataset. The y-axis is calculated by using @FAR_index_calc, and the x-axis is the score of the model inference on that example of noise. We can use this graph to calculate positive result thresholds for our classifier, at different false alarm rates. Once again, the models are listed with the number of artificial neurons in each hidden layer.]
) <perceptron_single_far>

Using this plot, we can select the score index that is closest to our desired FAR, and find the threshold which will generate a FAR of this value. With these threshold values in hand, we can then create efficiency curves at specific FARs. Efficiency curves allow us to examine the sensitivity of the model in detecting signals at different optimal SNR values. Where the sensitivity is given by

$ "sensitivity" = || "scores" > "score_threshold" || / || "scores" || $ <specificity>

where $|| "scores" > "score_threshold" ||$ is the number of scores above the score threshold, and $|| "scores" ||$ is the total number of examples tested. In @perceptron_efficiency_curves_single, we present the efficiency curves at three different values of FAR, #box("0.1"+ h(1.5pt) + "Hz"), #box("0.01"+ h(1.5pt) + "Hz"), and #box("0.001"+ h(1.5pt) + "Hz"), which are not particularly high FARs, but a can be seen from the plots, below these values we would encounter only negligible accuracies in the SNR ranges considered. As can be seen from the curves, the models do not perform well even with very generous FAR constraints.
...
#figure(
  ...
  caption: [Perceptron efficiency curves. For each of the 14 perceptron models trained, 31 efficiency tests were performed at evenly spaced optimal SNR values between 0 and 15. For each test, 8192 examples with signals of the relevant SNR were examined by the model, and the percentage of those that scored above the threshold was plotted, see @specificity, for three different thresholds: #box("0.1"+ h(1.5pt) + "Hz"), #box("0.01"+ h(1.5pt) + "Hz"), and #box("0.001"+ h(1.5pt) + "Hz"). The efficiency curve for each FAR threshold is presented on a unique plot. Some models have been excluded, they are shaded grey on the legends, because they are incapable of performing at the chosen FAR thresholds. _Upper:_ Efficiency curve at a False Alarm Rate (FAR) of #box("0.1" + h(1.5pt) + "Hz"). _Middle:_ Efficiency curve at #box("0.01" + h(1.5pt) + "Hz"). _Lower:_ Efficiency curve at #box("0.001" + h(1.5pt) + "Hz").]
) <perceptron_efficiency_curves_single>

Finally, we can view the model performance from a different dimension by freezing the SNR of the results and plotting the True Positive Rate (TPR), i.e. the sensitivity, against the False Alarm Rate (FAR). This will give us a Reciever Operator Curve (ROC) Curve, see @perceptron_roc_curve. We can compare the area under the curve for each model to make a comparison of its relative performance; although in this case, all the models perform very similarly at the chosen optimal SNR of 8. Chosen as this is often considered a good detectability threshold.

#figure(
  image("perceptron_single_roc.png", width: 100%), 
  caption: [Reciever Operator Curve (ROC) Curve at $rho_"opt" = 8$. All models show very similar, poor performance.]
) <perceptron_roc_curve> 

From these results, we can summarise that things are as anticipated from the results of the training. None of these models would have any useful application in gravitational-wave data science, as they all fall well below the performance of matched filtering. In order to offer any competitive approach, we must turn to other network architectures.
...
#figure(
  table(
    ...
    [George _et al._ @george_huerta_cnn], [0, 2, 4], [1, 3, 5], [6, 7], [No], [Single], [BBH], [No], [No], [$5 times 10^(-2)$], [0.98], [0.70], [0.16],
    [-], [0, 2, 4, 6], [1, 3, 5, 7], [8, 9, 10], [-], [-], [-], [-], [-], [-], [0.99], [0.80], [0.21],
    [George _et al._ @george_huerta_followup], [-], [-], [-], [ #text(red)[*Yes*] ], [-], [-], [-], [-], [-], [0.98], [0.77], [0.18],
    [Gabbard _et al._ @gabbard_messenger_cnn], [0, 1, 3, 4, 6, 7], [2, 5, 8], [9, 10, 11], [No], [Single], [BBH], [No], [#text(red)[*Yes*]], [$1 times 10^(-1)$], [1.0], [0.88], [0.44],
    [-], [-], [-], [-], [-], [-], [-], [-], [-], [$1 times 10^(-2)$], [0.99], [0.69], [0.10],
    [-], [-], [-], [-], [-], [-], [-], [-], [-], [$1 times 10^(-3)$], [0.98], [0.49], [0.02],
    [Fan _et al._ @multi_detector_fan_cnn], [0, 2, 4], [1, 3, 5], [6, 7], [No], [#text(red)[*Three*]], [BBH], [No], [No], [$4 times 10^(-2)$], [0.99], [0.84], [0.32],
    [Krastev _et al._ @krastev_bnn_cnn ], [0, 2, 4, 6], [1, 3, 5, 7], [8, 9, 10], [No], [Single], [#text(red)[*BNS*]], [No], [No], [$1 times 10^(-1)$], [0.71], [0.42], [0.20],
    [- ], [-], [-], [-], [-], [-], [-], [-], [-], [$1 times 10^(-2)$], [0.32], [0.10], [0.02],
    [- ], [-], [-], [-], [-], [-], [-], [-], [-], [$1 times 10^(-3)$], [0.11], [0.00], [0.00],
    [Gebhard _et al._ @gebhard_conv_only_cnn], [#text(red)[*Conv layers $times 12$*]], [None], [None], [#text(red)[*Yes*]], [#text(red)[*Two*]], [BBH], [No], [No], [800 (IFPR)], [0.83], [0.35], [Not Given],
    [-], [-], [-], [-], [-], [-], [-], [-], [-],[450 (IFPR)], [0.80], [0.32], [Not Given],
    [-], [-], [-], [-], [-], [-], [-], [-], [-], [100 (IFPR)], [0.67], [0.23], [Not Given],
    [Wang _et al._ @wang_cnn], [0, 2], [1, 3], [4, 5], [#text(red)[*Yes*]], [#text(red)[*Two*]], [BBH], [#text(red)[*Matched Filter*]], [No], [$1 times 10^(-1)$], [0.60], [0.24], [0.12],
    [-], [-], [-], [-], [-], [-], [-], [-], [-], [$1 times 10^(-2)$], [0.30], [0.05], [0.00],
    [-], [-], [-], [-], [-], [-], [-], [-], [-], [$1 times 10^(-3)$], [0.08], [0.00], [0.00],
    [Huerta _et al._ @huerta_fusion_cnn], [#text(red)[*Ensemble $times 4$*]], [None], [-], [Yes], [Two], [BBH], [No], [No], [#text(red)[*$5 times 10^(-4)$*]],  [0.20], [0.15], [Not Given],
...
#figure(
  grid(
    ...
    caption: [CNN architectures from George _et al._ @george_huerta_cnn. _Upper:_ Smaller model. _Lower:_ Larger model.]
) <george_diagram>
