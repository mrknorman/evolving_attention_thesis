...
#figure(
    ...
    caption: [Example projection of two artificial gravitational-wave waveforms. The blue waveforms have been projected into the LIGO Livingston interferometer, the red waveforms have been projected into the Ligo Hanford interferometer, and the green waveforms have been projected into the VIRGO interferometer. The left column displays different projections of an IMRPhenomD waveform generated with the cuPhenom GPU library; see @cuphenom-sec. The right column displays different projections of a WNB waveform generated with the method described in @injection-gen-sec. The projections are performed using a GPU adaptation of the PyCBC Python library's @pycbc project_wave function. Both waveforms are projected from different source locations; the projection and time displacement are different in each case. ]
) <projection_examples>
...
where $op("FAR")_(op("comb"))$ is the combined FAR, $N$ is the number of included detectors, $w$ is the duration of the input vector, and $o$ is the overlap between windows. This equation works in the case when a detection method tells you a feature has been detected within a certain time window, $w$, but not the specific time during that window, meaning that $t_"central" > w_"start" and t_"central" < w_"end"$, where $t_"central"$ is the signal central time, $w_"start"$ is the input vector start time and $w_"end"$ is the input vector end time. 

If a detection method can be used to ascertain a more constrained time for a feature ($w_"duration" < "light_travel_time"$), then you can use the light travel time between the two detectors to calculate a FAR. For two detectors, combing the FAR in this way can be achieved by
...
The obfuscating noise consists of real data taken from LIGO Hanford and LIGO Livingston for each respective detector. Locations of confirmed and candidate events are excluded from the data, but known glitch times have been included in the training, testing, and validation datasets.
...
When first reading about convolutional layers, it can be confusing to understand how they each "choose" which features to recognise. What should be understood is that this is not a manual process; there is no user input on which kernels filter which features; instead, this is all tuned by the chosen optimiser during the training process. Even the idea that each kernel will cleanly learn one feature type is an idealised simplification of what can happen during training. Gradient descent has no elegant ideas of how it should and should not use the architectures presented to it and will invariably follow the path of least resistance, which can sometimes result in strange and unorthodox uses of neural structures. The more complex and non-linear the recognition task, the more often this will occur. 
...
The result of using one or more convolutional layers on an input vector is an output vector with an extra discrete depth dimension, with each layer in the stack representing feature maps. Whilst often considerably more interpretable than maps of the parameters in dense layers, these maps are often not very useful alone. However, a flattened version of this vector is now, hopefully, much easier for dense layers to classify than the original image; see @flatten-sec. As such, CNNs used for classification are almost always capped by one or more dense layers in order to produce the final classification result; see @cnn_diagram for a toy example of a CNN used for binary classification.
...
#figure(
  table(
    ...
    [Gebhard _et al._ @gebhard_conv_only_cnn], [#text(red)[*Conv layers $times 12$*]], [None], [None], [#text(red)[*Yes*]], [#text(red)[*Two*]], [BBH], [No], [No], [$1.25  times 10^(-3)$], [0.83], [0.35], [Not Given],
...
We present an attempt to recreate the model and training procedure presented in George _et al._ @george_huerta_cnn and Gabbard _et al._, see @george_diagram and @gabbard_diagram respectively. The models were recreated as closely as possible, with the addition of GWFlow whitening layers as their first layer to replicate the data conditioning performed in both studies. These models will act as performance baselines as we move forward and try to improve on their operation.
...
Looking at the training plots, shown in @cnn_single_accuracy, it is quickly obvious that these models drastically outperform any of the perceptron models. This is perhaps unsurprising, as the CNN architecture was specifically designed for pattern recognition in time series. The training parameters are identical to those used for the single detector perceptron, see @perceptron-training-parameters. The models also train more rapidly, saturating their validation loss in fewer epochs than was required for the perceptrons to reach a lower accuracy, see @cnn_single_loss. This is also as expected, as more of the function that the networks are attempting to optimize has been predefined in the CNN case, where kernels can learn to recognize features across the image, as opposed to dense layers which would have to learn equivalent "kernels" individually for every possible feature position, and do so with far fewer examples for each of these unique kernels. Similar to the perceptron experiments, validation results are presented in @cnn-validation-sec.
...
The validation results portray a similar story --- vastly improved performance over the perceptron results. We can see in the FAR curve, @cnn_far_single, that we can utilize significantly lower False Alarm Rates (FARs) without dramatically increasing the required score threshold. We expect this to allow higher efficiencies at lower SNRs if the model has gained adequate detection ability. The benefit of this is displayed in the efficiency plots, @cnn_efficiency_curves_single, and the ROC plots, @roc_curves_single. 

#figure(
image("cnn_single/far_plot.png", width: 100%),
  caption: [False Alarm Rate (FAR) vs score threshold required to achieve that False Alarm Rate, for three recreations of models from the literature. two models from George _et al._ @george_huerta_cnn @george_huerta_followup, labeled "George Small", and "George Large", to differentiate them in terms of complexity, and the single model from Gabbard _et al._. The presented FAR curves are significantly lower than those achieved by the perceptrons in the single detector analysis case, see @perceptron_single_far. This means that we will be able to achieve lower FARs with lower score thresholds, which typically, though not necessarily, leads to higher efficiencies at the FARs. We explore the efficiency results in @cnn_efficiency_curves_single.]
) <cnn_far_single>
...
#figure(
  ...
  caption: [_First:_ Efficiency curves at a FAR of #box("0.1" + h(1.5pt) + "Hz"). _Second:_ Efficiency curves at a FAR of #box("0.01" + h(1.5pt) + "Hz"). _Third:_ Efficiency curves at a FAR of #box("0.001" + h(1.5pt) + "Hz"). _Fourth:_ Efficiency curves at a FAR of #box("0.0001" + h(1.5pt) + "Hz").]
...
It would appear from our investigation that CNNs offer a far superior solution to the single-detector CBC detection problem than perceptrons do. Several questions remain, however. Are we nearing the limit of the CNN capacity to solve this problem, or could further hyperparameter tuning, squeeze additional performance out of the architecture, especially at low False Alarm Rates? There have been many attempts to improve on these models throughout the literature, but there lacks a systematic algorithm to search across all possible solutions, (or at least a large number of possible solutions), to find the optimal detection architecture and training procedure. We investigate this in @dragonn-sec, where we attempt to use genetic algorithms to search the large hyperparameter space presented for more optimal solutions. There are also several more recently developed architectures, including attention-based models, which offer alternate and possibly superior alternatives to convolutional layers, we explore the use of attention layers for CBC classification in @skywarp-sec. Finally, there are other, potentially more challenging problems facing gravitational-wave data science, including parameter estimation. We tackle a special case wherein two overlapping signals are present in our input data; examining if we can separate the case of single and overlapping signal, and seeing if we can extract parameters from each signal, both in aid of alternate parameter estimation methods, and potentially as a precursor to a full machine learning based parameter estimator in @crosswave-sec.
