...
As has previously been noted, as well as a quasi-stationary coloured Gaussian background, interferometer noise also contains transient detector glitches caused by a plethora of sources, both known and unknown. These glitches have a prominent effect on the upper-sensitivity bound of most types of search, so it may be important to represent features of this type in our training pipeline. Previous experiments performed during the development of the MLy pipeline @MLy had shown that networks can often have greatly increased FARs when performing inference on data segments which contain transient glitches, even when those glitches were only present in the off-source segment used to generate the PSD used for data whitening. As such, a method to add glitches to the training distribution should be considered so that methods to deal with features of this type can hopefully be incorporated into the model's learned parameters during training.
...
#figure(
    ...
    caption: [Training loss history of perceptron models training to detect IMRPhenomD waveforms, generated using cuPhenom, obfuscated by real interferometer noise from the LIGO Livingston detector from the 3#super("rd") observing run. Visit #link("https://rawcdn.githack.com/mrknorman/data_ad_infinitum/c619a91b36513a2244ccd610c709bb1643a64b4c/models/chapter_04_perceptrons_single/training_history.html") for interactive plots, whilst they're still working.. The optimal SNR of waveforms injected into the training and validation sets was uniformly distributed between 8 and 15. Input was from a single detector only. A rough grid search was performed over a relatively arbitrary selection of model architectures, which varied the number of layers and the number of perceptrons in each layer. The architectures of each model can be seen in the figure legends as a list of numbers where each number is the number of artificial neurons in that layer. All were trained with the same training parameters, details of which can be found in the main text. Each epoch consisted of $10^5$ training examples, and it should be noted that, unlike the regular training pipelines, each training epoch consisted of new unseen examples injected into unseen noise segments, though the validation examples were consistent. Training of each model was halted after ten consecutive epochs with no improvement to validation loss, the values of which are shown in. Validation noise was drawn from a separate pool of GPS segments inaccessible to the training data loader. We can see that the accuracy only approaches 75%. Though this is from a pool of mixed SNRs and at an unrestrained false alarm rate (this accuracy uses a threshold of 0.5), this is clearly insufficient to be useful.  _Upper:_ Plot of model accuracies when mesured with training data ($10^5$ epoch unique examples). _Lower:_ Plot of model accuracies when mesured with validation data ($10^4$ epoch-consistent examples).]
...
#figure(
  ...
  caption: [Perceptron efficiency curves. _Upper:_ Efficiency curve at a False Alarm Rate (FAR) of #box("0.1" + h(1.5pt) + "Hz"). _Middle:_ Efficiency curve at #box("0.01" + h(1.5pt) + "Hz"). _Lower:_ Efficiency curve at #box("0.001" + h(1.5pt) + "Hz").]
)

#figure(
  image("perceptron_single_far_curves.png", width: 100%), 
  caption: [Perceptron False Alarm Rate (FAR) curves.]
)

#figure(
  image("perceptron_single_roc.png", width: 100%), 
  caption: [Reciever Operator Curve (ROC) Curve at $rho_"opt" = 8$. ]
)
